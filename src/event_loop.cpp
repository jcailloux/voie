...
char* uint_to_str(char* p, std::size_t val) noexcept {
    char tmp[20];
    int len = 0;
    if (val == 0) {
        *p = '0';
...
char* int_to_str(char* p, int val) noexcept {
    if (val < 0) {
        *p++ = '-';
...
event_loop::event_loop(const router& router, const app_config& config,
...
int event_loop::create_listen_socket(std::string_view address, std::uint16_t port) {
...
std::uint32_t event_loop::alloc_conn_id() {
...
void event_loop::free_conn_id(std::uint32_t id) {
...
void event_loop::on_recv(std::uint32_t conn_id, int res) {
    if (res <= 0) {
        start_close(conn_id);
        flush_io();
        return;
...
void event_loop::on_send(std::uint32_t conn_id, int res) {
    if (res <= 0) {
        start_close(conn_id);
        flush_io();
        return;
...
bool event_loop::should_close_connection(const parsed_request& req) {
...
void event_loop::process_request(std::uint32_t conn_id) {
    auto& conn = *connections_[conn_id];
...
void event_loop::update_date_cache() {
...
std::string_view event_loop::cached_date() noexcept {
    update_date_cache();
...
void event_loop::build_response(connection& conn, ctx& c) {
    auto& a = conn.alloc();
...
void event_loop::send_error_response(std::uint32_t conn_id, int status, std::string_view body) {
    auto& conn = *connections_[conn_id];
    auto& a = conn.alloc();
